...
export async function GET(req: NextRequest, ctx: { params: { market: string } }) {
  ...
  const cacheKey = `klaviyo-list-health:${market}:${range.start}:${range.end}:v1`;

  try {
    const result = await memo(cacheKey, TTL_6H, async () => {
      const lh = await listHealthAggregate(market, range);
      return { market, period: range, ...lh };
    });
    return NextResponse.json(result);
  } catch (err) {
    return NextResponse.json({ error: 'list-health failed', detail: (err as Error).message }, { status: 500 });
  }
}
