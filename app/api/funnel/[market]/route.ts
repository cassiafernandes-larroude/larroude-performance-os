  ...
  try {
    const result = await memo(`funnel:v1:${market}:${since}:${until}:${gran}`, TTL_10M, async () => {
      const [series, totals, payment, today] = await Promise.all([
        ...
        // Funil de HOJE (tempo real) via ShopifyQL ('today')
        getFunnelTotals(market, 'today', 'today').catch(() => null),
      ]);

      // Shares de cada etapa (período).
      const shares = {
      ...
      // Alerta tempo real: compara as taxas de HOJE com as do período; sinaliza queda anormal
      // (≥40%) numa etapa, desde que haja volume mínimo hoje.
      const alerts: Array<{ step: string; todayRate: number; periodRate: number; dropPct: number }> = [];
      if (today && today.sessions >= 100) {
        const steps: Array<{ key: string; label: string; tn: number; td: number; pn: number; pd: number }> = [
          { key: 'cart', label: 'Sessões → Carrinho', tn: today.addToCart, td: today.sessions, pn: totals.addToCart, pd: totals.sessions },
          { key: 'checkout', label: 'Carrinho → Checkout', tn: today.reachedCheckout, td: today.addToCart, pn: totals.reachedCheckout, pd: totals.addToCart },
          { key: 'completed', label: 'Checkout → Pedido', tn: today.completed, td: today.reachedCheckout, pn: totals.completed, pd: totals.reachedCheckout },
        ];
      ...
      return {
        available: true, market, since, until, gran,
        series, totals, shares, payment, today, alerts,
      };
    });
